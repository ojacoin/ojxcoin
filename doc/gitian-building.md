...
*Setup instructions for a gitian build of ojx using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ojx
...
Clone the git repositories for ojx and gitian and then checkout the ojx version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ojxcoin/ojx-core.git
cd ojx-core
...
Follow the instructions in [doc/release-process.md](release-process.md) in the ojx-core repository
...
To build ojx (for Linux, OSX, and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the ojx-core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ojx-core/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/ojxcoin/ojx-core
...
./bin/gbuild --commit ojx=${COMMIT} --url ojx=${URL} ../ojx/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ojx=${COMMIT} --url ojx=${URL} ../ojx/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ojx=${COMMIT} --url ojx=${URL} ../ojx/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ojx-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ojx-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/ojx-build.assert
```
...
[ojxcoin/gitian.sigs](https://github.com/ojxcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
