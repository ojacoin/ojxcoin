...
#if defined(NDEBUG)
#error "ojx cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "ojx Signed Message:\n";
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if(nHeight > 0 && nHeight <= 200) {
		...
		int64_t mNodeCoins = nMasternodeCount * 7500 * COIN;
		...
		if (mNodeCoins == 0) {
			...
			LogPrintf("[ojx] Adjusting ojx at height %d with %d masternodes (%d % locked Nodex) and %d Nodex supply at %ld\n", nHeight, nMasternodeCount, lockedCoinValue*100, nMoneySupply, GetTime());
			LogPrintf("[ojx] Masternode: %d\n", masternodeMultiplier*100);
			LogPrintf("[ojx] Staker: %d\n", (1 - masternodeMultiplier)*100);
